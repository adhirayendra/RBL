...
async function getAboutData(): Promise<AboutData | null> {
  return client.fetch<AboutData | null>(
    `*[_type == "about"][0]{ heroImage, heroImageAlt }`,
    {},
    { next: { revalidate: 60 } }
  );
}
...
export default async function AboutHero() {
  ...
  const imageSrc =
    data?.heroImage ? urlFor(data.heroImage).url() : "/image/about/about.png";
  const imageAlt = data?.heroImageAlt ?? "Radio Budi Luhur Team";
